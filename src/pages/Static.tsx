...
function Page({ crumb, a, b, text, aside, desc }: { crumb: string; a: string; b?: string; text: string; aside?: React.ReactNode; desc: string }) {
  useMeta([a, b].filter(Boolean).join(' '), desc)
  return (
    <>
      <Crumb items={[{ label: crumb }]} />
      <section className="wrap grid gap-10 pt-6 lg:grid-cols-[1fr_320px]">
        <div className="max-w-[820px]"><Heading a={a} b={b} as="h1" /><Markdown text={text} /></div>
...
export const Rules = () => <Page crumb="Κανονισμοί" a="Κανονισμοί" b="3on3" desc="Οι κανονισμοί του 3×3 όπως παίζονται στη GNC: 4 παίκτες ανά ομάδα, μισό γήπεδο, 10΄ ή πρώτος στους 21." text={kanonismoi} aside={
  <>
...
export const About = () => <Page crumb="Ποιοι είμαστε" a="Ποιοι" b="είμαστε" desc="Η GNC 3on3 από το 2018: δεκάδες πόλεις κάθε καλοκαίρι, δωρεάν συμμετοχή, επίσημοι κανονισμοί FIBA 3×3." text={aboutus} aside={
  <>
...
export const Volunteer = () => <Page crumb="Γίνε εθελοντής" a="Γίνε" b="εθελοντής" desc="Γίνε εθελοντής στη διοργάνωση της πόλης σου — γραμματεία, γήπεδο, φωτογραφία, υποδοχή ομάδων." text={ethelontis.replace(/Φόρμα συμμετοχής: https?:\/\/\S+/, '')} aside={
  <>
...
export const Terms = () => <Page crumb="Όροι συμμετοχής" a="Όροι" b="συμμετοχής" desc="Οι όροι συμμετοχής στις διοργανώσεις GNC 3on3." text={oroi} />
